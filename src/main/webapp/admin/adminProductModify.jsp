...
span, td, input {
	...
	<div class="content">
        <h2>상품수정</h2>
        <form action="adminProductModify" method="post" enctype="multipart/form-data" id="regForm">
        <input type="hidden" name="oldMainImage1" id="oldMainImage1" value="${pao.product.mainImage1}">
        <input type="hidden" name="oldMainImage2" id="oldMainImage2" value="${pao.product.mainImage2}">
        <input type="hidden" name="oldMainImage3" id="oldMainImage3" value="${pao.product.mainImage3}">
        <input type="hidden" name="oldMainImage4" id="oldMainImage4" value="${pao.product.mainImage4}">
        <input type="hidden" name="oldImage1" id="oldImage1" value="${pao.product.image1}">
        <input type="hidden" name="oldImage2" id="oldImage2" value="${pao.product.image2}">
        <input type="hidden" name="oldImage3" id="oldImage3" value="${pao.product.image3}">
        <input type="hidden" name="oldImage4" id="oldImage4" value="${pao.product.image4}">
        <input type="hidden" name="oldSizeChart" id="oldSizeChart" value="${pao.product.sizeChart}">
        <div class="container">
            <div class="left">
                 <div class="main-image" onclick="$('#mainImage1').click();">
                 	<span class="x" id="mian-x">✖</span>
                	<img id="main-image" 
                ...
                <div class="sub-thumbnails">
                    <div class="sub-thumb" onclick="$('#mainImage2').click();">
                    	<span class="x" id="sub-thumb1-x">✖</span>
                    	<img id="sub-thumb1" 
                    ...
                    <div class="sub-thumb" onclick="$('#mainImage3').click();">
                    	<span class="x" id="sub-thumb2-x">✖</span>
                    	<img id="sub-thumb2" 
                    ...
                    <div class="sub-thumb" onclick="$('#mainImage4').click();">
                    	<span class="x" id="sub-thumb3-x">✖</span>
                    	<img id="sub-thumb3" 
                ...
                <input type="file" id="mainImage1" accept="image/*" style="display:none" name="mainImage1" onchange="readURL(this,'main-image')" value="AA">
            ...
            <div class="right">
                <div class="form-box">
                    <div class="category-row">
                        <span class="pspan">카테고리</span>
                        <select id="categorySelect" name="category" class="cselect">
                            <c:forEach items="${categoryList }" var="category">
                            	<option value="${category.categoryId }" ${category.categoryId eq pao.product.categoryId? 'selected':'' }>${category.categoryName }</option>
                            </c:forEach>
                        </select>
                        <select  id="subCategorySelect" name="subCategory" class="cselect">
                            <c:forEach items="${subCategoryList }" var="subCategory">
                            	<option value="${subCategory.subCategoryId }" ${subCategory.subCategoryId eq pao.product.subCategoryId? 'selected':''}>${subCategory.subCategoryName }</option>
                            </c:forEach>
                        </select>
                    </div>

                    <div><span class="pspan">상품명</span><input type="text" name="name" placeholder="상품명 입력" class="dinput" required value=${pao.product.name }></div>
                    <div><span class="pspan">제품코드</span><input type="text" name="productId" placeholder="제품코드 입력" class="dinput" required value=${pao.product.productId }></div>
                    <div><span class="pspan">원가</span><input type="text" name="cost" placeholder="원가 입력" class="dinput" required value=${pao.product.cost }></div>
                    <div><span class="pspan">판매가</span><input type="text" name="price" placeholder="판매가 입력" class="dinput" required value=${pao.product.price }></div>
                    <div><span class="pspan">할인가</span><input type="text" name="discount" placeholder="할인가 입력" class="dinput" required value=${pao.product.discount }></div>
                </div>

                <input type='radio' value='F' name="sizeType" onclick="$('#options-container').empty()" ${pao.product.sizeType eq 'F'? 'checked':'' }><span>Free Size</span>
                <input type='radio' value='M' name="sizeType" onclick="$('#options-container').empty()" ${pao.product.sizeType eq 'M'? 'checked':'' }><span>Multi Size</span>
                    	...
                    	<c:forEach items="${pao.optionList }" var="option">
                    		<div class="color-block">
                    			<div class="option-row">
                    				<span>색 상</span>&nbsp;<input type='text' name='color' required value='${option.color}'>
                    				<span style='color:red' onclick="$(this).parent().parent().remove();">✖</span>
                    			</div>
                   					<table class="option_table" id="sizeTable">
        								...
        								<c:forEach items="${option.sizeQuanity }" var="sq">
        									<tr>
        										<td>${sq.size }</td>
        										<td><input type="number" name="${sq.size}" value="${sq.quantity }"></td>
        									</tr>
        								</c:forEach>
       								</table>
                    		</div>
        ...
        <div class="image-upload-section">
            <div class="big-preview-box" id="bigPreviewBox">
                <div class="big-img-slot" onclick="$('#imageDetail1').click(); $('#oldImage1').val('');">
                	<span class="x" id="image1-x">✖</span>
                	<img id="image1" 
                		src='${(pao.product.image1 eq null || pao.product.image1 eq "")? imageIcon:"image?filename="+=pao.product.image1 }'>
                </div>
                <div class="big-img-slot" onclick="$('#imageDetail2').click(); $('#oldImage2').val('');">
                	<span class="x" id="image2-x">✖</span>
                	<img id="image2" 
                		src='${(pao.product.image2 eq null || pao.product.image2 eq "")? imageIcon:"image?filename="+=pao.product.image2 }'>
                </div>
                <div class="big-img-slot" onclick="$('#imageDetail3').click(); $('#oldImage3').val('');">
                	<span class="x" id="image3-x">✖</span>
                	<img id="image3" 
                		src='${(pao.product.image3 eq null || pao.product.image3 eq "")? imageIcon:"image?filename="+=pao.product.image3 }'>
                </div>
                <div class="big-img-slot" onclick="$('#imageDetail4').click(); $('#oldImage4').val('');">
                	<span class="x" id="image4-x">✖</span>
                	<img id="image4" 
                		src='${(pao.product.image4 eq null || pao.product.image4 eq "")? imageIcon:"image?filename="+=pao.product.image4 }'>
                </div>
            </div>
            <input type="file" accept="image/*" style="display:none" name="image1" id="imageDetail1" onchange="readURL(this,'image1')">
            ...
            <input type="text" id="bigImageTitle" placeholder="제품 설명글 입력" name="description1" required value="${pao.product.description1 }"/>
            <div class="big-preview-box" id="sizeImageBox">
            	<div class="big-img-slot ${(pao.product.sizeChart eq null || pao.product.sizeChart eq '')? 'hide':'' }" id="sizeImageSlot"><span class="x" id="sizeImage-x">✖</span>
            		<img id="sizeImage" 
            			src="${(pao.product.sizeChart eq null || pao.product.sizeChart eq '')? imageIcon:'image?filename='+=pao.product.sizeChart }"></div>
            	</div>
            <input type="file" id="sizeImageInput" accept="image/*" style="display:none" name="sizeChart" onchange="sizeImageChange(this,'sizeImage')">
            <span class="add-btn" id="sizeImageUploadBtn" onclick="$('#sizeImageInput').click()">+ 사이즈 설명 등록</span>
        </div>
