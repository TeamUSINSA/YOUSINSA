	...
	<div class="container">
		...
		<table class="info-table">
			...
			<tr>
				...
				<td>${member.address1} ${member.address1Detail}<br />
			...
			<c:if test="${not empty member.withdrawalReason}">
				<tr>
					...
					<td>${member.withdrawalReason}<br />
						(${member.withdrawalDetail})
					</td>
				</tr>
			</c:if>
		</table>
					...
					<tr onclick="goToOrderDetail(${order.orderId})">
						...
						<td><c:choose>
								<c:when test="${order.deliveryStatus == '배송완료'}">
									<span class="status-complete">${order.deliveryStatus}</span>
								</c:when>
								<c:when test="${order.deliveryStatus == '배송중'}">
									<span class="status-shipping">${order.deliveryStatus}</span>
								</c:when>
								<c:otherwise>
									<span class="status-waiting">${order.deliveryStatus}</span>
								</c:otherwise>
							</c:choose></td>
					</tr>
				</c:forEach>
