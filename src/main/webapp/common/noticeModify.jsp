...
<head>
  ...
  <style>
    ...
    .container { max-width:600px; margin:40px auto; padding:24px; background:#fff; border-radius:8px; box-shadow:0 2px 8px rgba(0,0,0,0.1); }
    ...
    .btn { display:inline-block; padding:8px 16px; margin-left:8px; text-decoration:none; font-size:14px; border-radius:4px; border:1px solid #bbb; color:#333; background:#eee; }
      ...
      <div class="form-group">
        ...
        <div class="preview">
          <c:if test="${not empty notice.image}">
            <img id="preview" src="${pageContext.request.contextPath}/image/${notice.image}" alt="현재 이미지"/>
          </c:if>
          <c:if test="${empty notice.image}">
