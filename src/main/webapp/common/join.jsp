...
<main class="flex flex-col items-center justify-center py-24">
  ...
  <form id="joinForm" action="<%=path%>/join" method="post">
    ...
    <section class="mb-10">
      ...
      <div class="space-y-2 text-sm">
        ...
        <div>
          <input type="checkbox" name="terms2" class="terms" /> 개인정보 수집·이용 동의(필수)
        </div>
        <div>
          <input type="checkbox" name="terms3" class="terms" /> 마케팅 수신동의(선택)
          <div class="ml-6 mt-1 text-xs">
            <input type="checkbox" name="emailAgree" class="terms" /> 이메일 
            <input type="checkbox" name="smsAgree" class="ml-4 terms" /> SMS
          </div>
        </div>
      </div>
    </section>
    ...
    <section>
      ...
      <div class="space-y-4 text-sm">
        ...
        <input type="password" id="confirmPassword"
          ...
          class="w-full border px-3 py-2 rounded" required />
        <div id="pwCheckMsg" class="text-xs mt-1"></div>
        ...
        <div class="flex space-x-2 items-center">
          <input type="text" name="email_id" placeholder="이메일 아이디"
            class="w-1/2 border px-3 py-2 rounded" required />
          <span>@</span>
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    form.addEventListener("submit", function (e) {
      if (password.value !== confirmPassword.value) {
        ...
        pwCheckMsg.className = "text-xs mt-1 text-red-600";
    ...
    confirmPassword.addEventListener("input", function () {
      if (password.value === confirmPassword.value) {
        ...
        pwCheckMsg.className = "text-xs mt-1 text-green-600";
      } else {
        pwCheckMsg.textContent = "비밀번호가 일치하지 않습니다.";
        pwCheckMsg.className = "text-xs mt-1 text-red-600";
      }
