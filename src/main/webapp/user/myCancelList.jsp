...
<style>
  .filter-box { border:1px solid #ccc; border-radius:10px; padding:20px; margin-bottom:30px; background:#fafafa; }
  .month-buttons { display:flex; gap:10px; margin-bottom:15px; }
  .month-buttons input[type="button"] { padding:8px 16px; border:1px solid #ccc; background:#fff; border-radius:6px; font-size:14px; cursor:pointer; }
  .month-buttons input[type="button"]:hover, .month-buttons input[type="button"].active { background:#8cc63f; color:#fff; }
  .date-selects { display:flex; align-items:center; gap:8px; }
  .date-selects select { padding:6px; border:1px solid #ccc; border-radius:5px; font-size:14px; }
  .date-selects button { margin-left:10px; background:#030303; color:#fff; border:none; padding:8px 18px; border-radius:5px; cursor:pointer; }

  .order-header { display:flex; font-weight:bold; background:#f9f9f9; border-top:2px solid #444; border-bottom:1px solid #ccc; font-size:14px; padding:12px 0; }
  ...
  .order-header .quantity, .order-header .price, .order-header .status { flex:1; }

  .order-group { display:flex; border-bottom:2px solid #ccc; }
  .order-meta { width:160px; display:flex; flex-direction:column; justify-content:center; align-items:center; text-align:center; font-size:13px; padding:10px; border-right:1px solid #eee; line-height:1.8; }
  .order-items { flex:1; }
  .order-item { display:flex; padding:20px; align-items:center; border-top:1px solid #eee; }
  .order-item:first-of-type { border-top:none; }
  .product-info { flex:4; display:flex; align-items:center; gap:10px; }
  .order-image { width:60px; height:60px; object-fit:cover; border:1px solid #ccc; }
  .product-name { font-weight:bold; text-decoration:none; color:#333; }
  .product-name:hover { text-decoration:underline; }
  .quantity, .price, .status { flex:1; text-align:center; }
  .detail-btn { padding:4px 10px; font-size:12px; margin-top:6px; border:1px solid #8cc63f; background:#fff; color:#8cc63f; border-radius:4px; cursor:pointer; text-decoration:none; transition:0.2s; }
...
<div style="max-width:1200px; margin:0 auto; display:flex;">
  ...
  <div style="flex:1; padding:40px 20px;">
    <h2 style="font-size:20px; font-weight:bold; margin-bottom:20px;">취소/반품 내역</h2>
    <p style="font-size:13px; color:#666;">귀하께서 취소 또는 반품하신 내역의 상품들입니다.</p>

    <form method="get" action="${pageContext.request.contextPath}/myCancelList" id="dateForm">
        ...
        <div class="date-selects">
          ...
          <select name="startMonth">
            <c:forEach var="m" begin="1" end="12">
              ...
              <option value="${mm}" <c:if test="${fn:substring(startDate,5,7)==mm}">selected</c:if>>${m}월</option>
            </c:forEach>
          </select>
          <select name="startDay">
            <c:forEach var="d" begin="1" end="31">
              <fmt:formatNumber var="dd" value="${d}" pattern="00"/>
              <option value="${dd}" <c:if test="${fn:substring(startDate,8,10)==dd}">selected</c:if>>${d}일</option>
            </c:forEach>
          </select>
          ~
          <select name="endYear">
          ...
          <select name="endMonth">
            <c:forEach var="m" begin="1" end="12">
              <fmt:formatNumber var="mm" value="${m}" pattern="00"/>
              <option value="${mm}" <c:if test="${fn:substring(endDate,5,7)==mm}">selected</c:if>>${m}월</option>
            </c:forEach>
          </select>
          <select name="endDay">
            <c:forEach var="d" begin="1" end="31">
              <fmt:formatNumber var="dd" value="${d}" pattern="00"/>
              <option value="${dd}" <c:if test="${fn:substring(endDate,8,10)==dd}">selected</c:if>>${d}일</option>
            </c:forEach>
          </select>
    ...
    <div class="order-header">
      <div class="meta"><span>주문일자</span><br><span>취소/반품일자</span></div>
              ...
              <div class="order-item">
                <div class="product-info">
                  <a href="${pageContext.request.contextPath}/productDetail?productId=${item.productId}">
                    <img src="${pageContext.request.contextPath}/image/${item.mainImage1}" class="order-image"/>
                  </a>
                  <div>
